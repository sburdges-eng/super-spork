...
class WavUtils {
  ...
  readWav(filePath) {
    try {
      ...
      const bytesPerSample = this._bytesPerSample(wav.bitDepth);
      const totalSampleValues = bytesPerSample
        ? (wav.data.samples.length / bytesPerSample)
        : null;
      const totalFrames = (totalSampleValues && wav.fmt.numChannels)
        ? (totalSampleValues / wav.fmt.numChannels)
        : null;
      ...
      return {
        ...
        metadata: {
          ...
          frames: totalFrames,
          sampleValues: totalSampleValues
        }
      };
    } catch (error) {
      return {
  ...
  getDuration(wav) {
    ...
    const totalSampleValues = wav.data.samples.length / bytesPerSample;
    const totalFrames = totalSampleValues / wav.fmt.numChannels;
    return totalFrames / wav.fmt.sampleRate;
  }
  ...
  convertChannels(wav, targetChannels) {
    const sourceChannels = wav.fmt.numChannels;
    if (targetChannels === sourceChannels) {
      return new WaveFile(wav.toBuffer());
    }

    const sampleRate = wav.fmt.sampleRate;
    const bitDepth = parseInt(wav.bitDepth);
    ...
    if (targetChannels === 1 && sourceChannels === 2) {
      ...
      for (let i = 0; i < samples.length; i += 2) {
        mono.push(Math.round((samples[i] + samples[i + 1]) / 2));
      }
      return this.createWav(mono, { sampleRate, bitDepth, numChannels: 1 });
    }
    ...
    if (targetChannels === 2 && sourceChannels === 1) {
      ...
      for (let i = 0; i < samples.length; i++) {
        stereo.push(samples[i], samples[i]);
      }
      return this.createWav(stereo, { sampleRate, bitDepth, numChannels: 2 });
    }

    throw new Error(`Unsupported channel conversion: ${sourceChannels} -> ${targetChannels}`);
  }
  ...
  trim(wav, startTime, endTime) {
    ...
    const samples = Array.from(wav.getSamples(true));
  ...
  concatenate(wavFiles) {
    ...
    wavFiles.forEach(wav => {
      ...
      const samples = processedWav.getSamples(true);
      for (let i = 0; i < samples.length; i++) {
        combinedSamples.push(samples[i]);
      }
    });
  ...
  mix(wavFiles, volumes = null) {
    ...
    const normalizedSamples = normalizedWavs.map(wav => wav.getSamples(true));

    // Find maximum length (avoid spread on large arrays)
    let maxLength = 0;
    for (let i = 0; i < normalizedSamples.length; i++) {
      if (normalizedSamples[i].length > maxLength) maxLength = normalizedSamples[i].length;
    }

    // Mix samples
    const mixedSamples = new Array(maxLength).fill(0);

    normalizedSamples.forEach((samples, index) => {
    ...
    for (let i = 0; i < mixedSamples.length; i++) {
      const absValue = Math.abs(mixedSamples[i]);
    ...
    for (let i = 0; i < mixedSamples.length; i++) {
      mixedSamples[i] = Math.round(mixedSamples[i]);
    }
  ...
  analyze(wav) {
    ...
    // Find peak + detect clipping (single pass; avoids large array spreads)
    let peak = 0;
    const maxValue = Math.pow(2, parseInt(wav.bitDepth) - 1) - 1;
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      if (absValue >= maxValue) clippedSamples++;
    }
  ...
  extractChannel(wav, channel = 0) {
    ...
    const samples = [];
    const source = wav.getSamples(true);
    for (let i = channel; i < source.length; i += wav.fmt.numChannels) {
      samples.push(source[i]);
    }
